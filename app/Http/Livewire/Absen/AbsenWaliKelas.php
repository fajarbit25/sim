...
class AbsenWaliKelas extends Component
{
    ...
    {
        return view('livewire.absen.absen-wali-kelas');
    }

    public function getDataKelas()
    {
        $data = Room::where('wali_kelas', Auth::user()->id)->where('campus_id', Auth::user()->campus_id)->get();
        $this->dataKelas = $data;
    }
    ...
    public function generateAbsensi()
    {
        ...
        foreach($users as $user){
            $cek = Absen::where('siswa_id', $user->id)->where('ta', $this->ta)->where('semester', $this->semester)
                    ->where('kelas', $this->kelas)->where('mapel', 0)->where('tanggal_absen', $this->tanggal)
    ...
    public function getCount()
    {
        ...
        $data = Absen::where('ta', $this->ta)->where('semester', $this->semester)
                ->where('absens.kelas', $this->kelas)->where('mapel', 0)
