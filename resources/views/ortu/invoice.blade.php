                ...
                <div class="card recent-sales overflow-auto">
                    ...
                    <div class="card-body">
                      ...
                      <ol class="list-group list-group-numbered">
                        ...
                        <a href="/user/invoice/{{$inv->nomor_invoice}}/show">
                          <li class="list-group-item d-flex justify-content-between align-items-start">
                            <div class="ms-2 me-auto">
                              <div class="fw-bold">Rp.{{number_format($inv->amount)}}</div>
                              <span class="fw-bold text-success">#{{$inv->nomor_invoice}}</span> 
                              Pembayaran {{$inv->tipe}}
                            </div>
                            <span class="badge bg-secondary rounded-pill">{{$inv->invoice_date}}</span>
                              ...
                              @if($inv->invoice_status == 'CANCELLED')<span class="badge bg-danger rounded-pill">{{$inv->invoice_status}}</span>@endif
                          </li>
                        </a>
                        @endforeach
                      </ol>
